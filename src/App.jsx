...
/* ============================== HOST ACCESS ============================== */
// Simple client-side gate so only someone who knows this passcode can create new rooms.
// Anyone with the game link can still join an existing room with a code — this only
// restricts who can spin up new games.
const HOST_PASSCODE = "Hamed3299931";
...
function startHand(room) {
  const remainingPlayers = room.players.filter((p) => !p.left);
  const hostStillHere = remainingPlayers.some((p) => p.id === room.hostId);
  const hostId = hostStillHere ? room.hostId : (remainingPlayers[0]?.id || room.hostId);
  room = { ...room, players: remainingPlayers, hostId };
...
function leaveRoom(room, playerId) {
  ...
  const inActiveHand = ["preflop", "flop", "turn", "river"].includes(room.phase) && room.seatOrder.includes(playerId);

  function reassignHost(r) {
    if (r.hostId !== playerId) return r;
    const nextHost = r.players.find((p) => p.id !== playerId);
    return { ...r, hostId: nextHost ? nextHost.id : r.hostId };
  }

  if (!inActiveHand) {
    // Safe to remove right away: lobby, between hands, or wasn't dealt into this hand.
    let r = {
      ...room,
      players: room.players.filter((p) => p.id !== playerId),
      seatOrder: room.seatOrder.filter((id) => id !== playerId),
      log: [...room.log, `${player.name} اتاق را ترک کرد.`],
    };
    return reassignHost(r);
  }

  if (player.folded || player.allIn) {
    // Already out of the betting this hand — just flag for cleanup once the hand ends.
    let r = { ...room, players: room.players.map((p) => (p.id === playerId ? { ...p, left: true } : p)), log: [...room.log, `${player.name} اتاق را ترک کرد.`] };
    return reassignHost(r);
  }

  // Mid-hand and still active: force-fold them, then let the normal round logic advance play.
  const players = room.players.map((p) => (p.id === playerId ? { ...p, folded: true, left: true } : p));
  const actedSet = [...room.actedSet, playerId];
  let r = { ...room, players, actedSet, log: [...room.log, `${player.name} اتاق را ترک کرد (فولد شد).`] };
  r = reassignHost(r);
  r = advance(r);
  return r;
}
        ...
        <div style={{ background: "rgba(0,0,0,.25)", border: "1px solid #2d5641", borderRadius: 14, padding: 20 }}>
          ...
          {mode === "create" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div>
                <label style={{ color: "#c9d9cf", fontSize: 13, marginBottom: 6, display: "block" }}>رمز میزبان</label>
                <input style={inputStyle} type="password" value={passcode} onChange={(e) => setPasscode(e.target.value)} placeholder="رمز رو وارد کن" />
              </div>
              <button style={{ ...btnStyle, opacity: busy ? 0.6 : 1 }} disabled={busy} onClick={() => {
                if (passcode !== HOST_PASSCODE) { setError("رمز میزبان اشتباهه."); return; }
                setError("");
                onCreate(name.trim(), { startingChips: chips, smallBlind: sb });
              }}>{busy ? "در حال ساخت..." : "ساخت اتاق"}</button>
...
function TableScreen({ room, myId, onAction, onLeave }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#081a10", display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 16px", borderBottom: "1px solid #1e3d2c" }}>
        ...
        <button
          onClick={() => { if (window.confirm("مطمئنی می‌خوای اتاق رو ترک کنی؟")) onLeave(); }}
          style={{ background: "none", border: "1px solid #4a2a2a", color: "#c98f7a", borderRadius: 6, padding: "4px 10px", fontSize: 11, cursor: "pointer" }}
        >ترک اتاق</button>
      </div>
...
const SESSION_KEY = "poker-hamed-session";
...
function PokerApp() {
  const initialSession = useState(() => loadSession())[0];
  const [myId, setMyId] = useState(() => initialSession?.id || randomId());
  const [myName, setMyName] = useState(() => initialSession?.name || "");
  const [room, setRoom] = useState(null);
  const [error, setError] = useState("");
  const [busy, setBusy] = useState(true); // true on first mount while we try to resume a session
  ...
  // Try to resume a previous session (e.g. after a page refresh) on first load.
  useEffect(() => {
    (async () => {
      if (!initialSession?.code || !initialSession?.id) { setBusy(false); return; }
      const latest = await loadRoom(initialSession.code);
      if (latest && !latest.__error && latest.players.some((p) => p.id === initialSession.id)) {
        setRoom(latest);
        startListeningRoom(initialSession.code);
      } else {
        clearSession();
      }
      setBusy(false);
  ...
  async function handleJoin(name, code) {
    ...
    try {
      ...
      setRoom(updated);
      startListeningRoom(code);
      saveSession({ code, name, id: joinedId });
    } catch (e) {
      setError("یه خطای غیرمنتظره پیش اومد: " + (e?.message || String(e)));
    } finally {
      setBusy(false);
  ...
  async function handleLeave() {
    if (room) {
      ...
      if (latest && !latest.__error) {
        const updated = leaveRoom(latest, effectiveMyId);
  ...
  if (busy && !room) {
    return <div style={{ minHeight: "100vh", background: "#081a10" }} />;
  }
  ...
  return <TableScreen room={room} myId={effectiveMyId} onAction={handleAction} onLeave={handleLeave} />;
}
